...
function ScreenOrientationAPI_lock(orientations,     // @arg OrientationString|OrientationStringArray - orientation or [orientation, ...]
                                   lockedCallback,   // @arg Function = null - lockedCallback():void
                                   failedCallback) { // @arg Function = null - failedCallback():void
//{@dev
    if (VERIFY) {
        ...
        $valid($type(lockedCallback, "Function|omit"), ScreenOrientationAPI_lock, "lockedCallback");
        $valid($type(failedCallback, "Function|omit"), ScreenOrientationAPI_lock, "failedCallback");
    }
//}@dev

    lockedCallback = lockedCallback || function() {};
    failedCallback = failedCallback || function() {};

    if (ENABLE_ORIENTATION_API) {
        if (_orientationObject["lock"]) {
            _orientationObject["lock"](orientations).then(function() {
            ...
            var api = "lockOrientation"    in _orientationObject ? "lockOrientation"
                    : "msLockOrientation"  in _orientationObject ? "msLockOrientation"
                    : "mozLockOrientation" in _orientationObject ? "mozLockOrientation" : "";
            if (api) {
...
function ScreenOrientationAPI_unlock() {
    if (ENABLE_ORIENTATION_API) {
        if (_orientationObject["unlock"]) {
            _orientationObject["unlock"]();
            _isScreenLocked = false;
        } else {
            var api = "unlock"               in _orientationObject ? "unlock"
                    // https://www.w3.org/TR/2014/WD-screen-orientation-20140220/
                    // https://www.w3.org/TR/2012/WD-screen-orientation-20121206/
                    : "unlockOrientation"    in _orientationObject ? "unlockOrientation"
                    : "msUnlockOrientation"  in _orientationObject ? "msUnlockOrientation"
                    : "mozUnlockOrientation" in _orientationObject ? "mozUnlockOrientation"
                    : "";
